...
class TestPrepareQuery {
  ...
  def initialize() = {
    ...
    diskTestQuery = "SELECT NON_NEGATIVE_DERIVATIVE(MEAN(\"readBytes\"), 1h) / 3600 AS \"readBytes\", NON_NEGATIVE_DERIVATIVE(MEAN(\"writeBytes\"), 1h) / 3600 AS \"writeBytes\", NON_NEGATIVE_DERIVATIVE(MEAN(\"readIOPS\"), 1h) / 3600 AS \"readIOPS\", NON_NEGATIVE_DERIVATIVE(MEAN(\"writeIOPS\"), 1h) / 3600 AS \"writeIOPS\", NON_NEGATIVE_DERIVATIVE(MEAN(\"ioErrors\"), 1h) / 3600 AS \"ioErrors\" FROM \"disk\" WHERE \"vmUuid\" = '87b753e0-da9d-474c-a7fa-9c942ed2a0ea' AND \"image\" = '938c8420-3e42-4b34-96a9-8479ef9d0f88' AND time > now() - 1d - 1d AND time < now() - 1d GROUP BY time(1h)"
